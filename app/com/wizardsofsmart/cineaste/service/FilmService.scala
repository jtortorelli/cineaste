...
class FilmService @Inject()(filmRepository: FilmRepository, photoService: PhotoService) {
  ...
  def film(uuid: String): Future[Either[DomainError, (Film, Seq[FilmStaff], Seq[FilmCast], Seq[FilmStudio], Option[FilmSeries], String, Option[List[Photo]])]] = {
  ...
  private def getGallery(uuid: String): Option[List[Photo]] = {
    photoService.retrieveGallery(s"public/images/galleries/films/$uuid")
  }
